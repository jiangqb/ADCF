function [res_feat poses_in_patch]= extract_features_STRCF(image, pos, scales, features, gparams, extract_info, wintype)
...
for sz_ind = 1:num_sizes
    ...
    for scale_ind = 1:num_scales
        ...
        pos_in_patch = round(1+[img_input_sz(1) img_input_sz(2)]/2);
        multires_pos_template=[multires_pos_template;pos_in_patch];
    end
    poses_in_patch{sz_ind}=round(multires_pos_template/gparams.cell_size);
end
...
if(CNN_ind ~=-1)
    ...
    if(num_features == 3)
        res_feat{1} = cat(3,feature_map{1}, feature_map{2}, feature_map{3});  
    elseif (num_features == 1) % by shuiwang
...
res_feat{i} = bsxfun(@times,ft,cos_window);
end
%----------------------------
