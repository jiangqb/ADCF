...
function results=run_ADCF_tracker(seq, res_path, bSaveImage, parameters)
setup_paths();
...
% Filter parameters
params.search_area_shape = 'square';  % important  % the shape of the training/detection window: 'proportional', 'square' or 'fix_padding'
params.search_area_scale = 3; %important        % the size of the training/detection area proportional to the target size
params.filter_max_area = 50^2;          % the size of the training/detection area in feature grid cells

% Learning parameters
% params.learning_rate = 0.02;	%very important		% learning rate
...
params.interpolate_response = 4;  %important   % correlation score interpolation strategy: 0 - off, 1 - feature grid, 2 - pixel grid, 4 - Newton's method
...
% params.gt_rects=seq.gt_rects;

results = ADCF_tracker(params);
